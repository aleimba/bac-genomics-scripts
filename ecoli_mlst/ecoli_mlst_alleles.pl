...
ecoli_mlst_alleles.pl                                   25-10-2011

=head1 SYNOPSIS

The script searches for MLST alleles in E. coli genomes according to
Mark Achtman's scheme with seven house-keeping genes (adk, fumC, gyrB,
icd, mdh, purA, and recA) [Wirth et al., 2006]. NUCmer from the MUMmer
package (http://mummer.sourceforge.net/) is used to compare the given
allele sequences to bacterial genomes via nucleotide alignments. The
...
into a multi-fasta file. Option B<--c> can be used to initiate an
alignment for this multi-fasta file with ClustalW (standard alignment
parameters; has to be in the C<$PATH> or change variable
C<$clustal_call>). The alignment output file can be used directly for
I<RAxML>.

I<ecoli_mlst_alleles.pl> works with draft and complete genomes,
however only for those genomes whose allele sequences have been
deposited in Achtman's allele database. If the alleles cannot be found
use I<run_sub_seq.pl> to get the corresponding 'new' allele sequences.

Alleles that were not found in a genome are marked by a '?' in the
result profile file and a place holder 'XXX' in the result fasta file.
...
All files have to be deposited in the folder in which the script
is run. Non-NCBI fasta headers for the genome files have to have a
unique ID directly following the '>'.
...
=item B<-a>, B<-alleles>     File extension of the MLST allele fasta files, e.g. 'fas'.

=item B<-g>, B<-genomes>     File extension of the E. coli genome fasta files, e.g. 'fasta'.

=back

=head2 Optional option

=over

=item B<--h>, B<--help>      Help (perldoc POD).

=item B<--c>, B<--clustalw>  Call ClustalW for alignment.
...
=item C<perl ecoli_mlst_alleles.pl -a fas -g fasta [-c]>

=back

=head1 VERSION

0.2                                                update: 20-10-2012
...
### ToDo
# - blastn alignments to control allele hits?
# - NUCmer/blastn also to find alleles not yet present in Achtman's DB, get the coordinates from the blast output and subsequently the alleles from the respective E. coli genomes? --> 'get_sub_seq.pl'


### Get the options with Getopt::Long, works also abbreviated and with two "--": -g, --g, -genomes ...
my $usage = "perldoc $0";
die system($usage) unless @ARGV;
my $allele_ext = ''; # file extension of allele fasta files
my $genome_ext = ''; # file extension of E. coli genome fasta files
...
### Control for multi-fasta genome files and concatenate them to a single-fasta entry (e.g. draft genomes), concatenate these and all other E. coli genome sequence files to one multi-fasta/-genome file for the subsequent nucmer run; additionally parse and associate the E. coli accession#s/unique IDs and descriptions (strain names ...) from the genomes
my $genome_file = 'Ecoli_multi.fasta'; # the multi-genome/-fasta file for the nucmer run, used as a temp file, will be deleted at the end of the script
...
foreach my $file (@genome_files) {
    ...
    my ($acc, $desc) = acc_desc($new_id); # subroutine to fill hash %genome_desc and get acc# and genome description
...
my %strain_allele; # declare hash, that's subsequently used as anonymous 'hash in hash' in sub parse_coords to associate MLST alleles and accession#s
...
if ($clustalw) {
    ...
    $out =~ s/\.fasta$//;
    my $clustal_call = "clustalw -infile=$ecoli_allele_seq -outfile=$out\_aln.fasta -align -type=DNA -output=phylip -tree -newtree=$out\_aln.dnd -outputtree=phylip";
    system ($clustal_call);
}
...
sub parse_coord {
    ...
    while (<COORD>) {
        ...
        $strain_allele{$allele}{$6} = $5; # associate allele# with acc# and store as anonymous hash in declared hash %strain_allele
    }
    }
    close COORD;
