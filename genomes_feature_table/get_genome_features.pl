...
use warnings;
use strict;
use autodie; # pragma to control if functions work, or die with lexical scope
...
my $usage = "\n".
   ...
   "\t# Results are written to a tab-separated file '*_features.txt'.  #\n".
   ...
   "\t# To be on the safe side, put each individual genome (including  #\n".
   "\t# all replicons) in a seperate file.                             #\n".
   "\t# The perl script uses BioPerl (www.bioperl.org).                #\n".
   "\t#                                                                #\n".
   "\t# version 0.4, update: 11.08.2013                     A Leimbach #\n".
...
if (!defined $extension) {
    die $usage, "###Fatal error: A file extension is not given!\n\n";
} elsif ($extension =~ /-h/) {
    die $usage;
} elsif ($extension =~ /-v/) {
    warn "$0 $version\n";
...
if (defined $option && $option ne 'p') {
    die $usage, "\n###Fatal error: Option \'$option\' given instead of \'p\'!\n\n";
}
