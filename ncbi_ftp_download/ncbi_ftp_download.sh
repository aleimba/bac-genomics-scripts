...
wget -cNrv -t 45 -A *.gbk,*.fna "ftp://ftp.ncbi.nih.gov/genomes/Bacteria/$1*" -P .
# Download/update RefSeq draft genomes
echo "#### Updating RefSeq draft $1 genomes"
wget -cNrv -t 45 -A *.gbk.tgz,*.fna.tgz "ftp://ftp.ncbi.nih.gov/genomes/Bacteria_DRAFT/$1*" -P .
# Download/update GenBank complete genomes
echo "#### Updating GenBank complete $1 genomes"
wget -cNrv -t 45 -A *.gbk,*.fna "ftp://ftp.ncbi.nih.gov/genbank/genomes/Bacteria/$1*" -P .
# Download/update GenBank draft genomes
echo "#### Updating GenBank draft $1 genomes"
wget -cNrv -t 45 -A *.gbk.tgz,*.fna.tgz "ftp://ftp.ncbi.nih.gov/genbank/genomes/Bacteria_DRAFT/$1*" -P .
