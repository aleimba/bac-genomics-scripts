#!/bin/bash
echo "###Running cds_extractor.pl"
for i in *.$1; do perl cds_extractor.pl -s $i -p -f; done
cat *_cds_aa.fasta > blast_finder_aa.fasta
rm -f *_cds_aa.fasta
echo "###Running legacy BLASTP"
...
echo "###Running blast_prot_finder.pl plus Clustal-Omega alignment"
